import Link from "next/link"
import { Instagram, Facebook } from "lucide-react"

const treatments = [
  "Rituales Faciales Personalizados",
  "Micropigmentación Artística",
  "Arquitectura de Cejas",
  "Escultura Corporal",
  "Rejuvenecimiento Avanzado",
  "Lifting & Tinte de Pestañas",
]

export function Footer() {
  return (
    <footer className="pt-16 pb-10 bg-foreground text-card">
      <div className="container mx-auto px-6 lg:px-12">
        {/* Top grid */}
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-14 pb-14 border-b border-card/10">

          {/* Brand */}
          <div className="lg:col-span-1">
            <Link href="/" className="font-serif text-xl font-light tracking-wider text-card">
              YULI COLORS
            </Link>
            <p className="text-card/50 mt-4 text-sm leading-relaxed max-w-xs">
              Centro de estética premium dedicado a realzar tu belleza natural con
              tratamientos personalizados y resultados que perduran.
            </p>
            {/* Social */}
            <div className="flex gap-3 mt-6">
              <Link
                href="https://www.instagram.com/yuli_colors/"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 flex items-center justify-center border border-card/15 hover:border-primary hover:text-primary transition-all duration-300"
                aria-label="Instagram de YULI COLORS"
              >
                <Instagram className="w-4 h-4" />
              </Link>
              <Link
                href="https://facebook.com/yulicolors"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 flex items-center justify-center border border-card/15 hover:border-primary hover:text-primary transition-all duration-300"
                aria-label="Facebook de YULI COLORS"
              >
                <Facebook className="w-4 h-4" />
              </Link>
              {/* WhatsApp */}
              <Link
                href="https://wa.me/34622886878?text=Hola,%20me%20gustaría%20reservar%20una%20cita%20en%20YULI%20COLORS"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 flex items-center justify-center border border-card/15 hover:border-[#25D366] hover:text-[#25D366] transition-all duration-300"
                aria-label="WhatsApp de YULI COLORS"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
              </Link>
            </div>
          </div>

          {/* Navigation */}
          <div>
            <h4 className="text-[10px] tracking-[0.3em] uppercase text-card/40 mb-5 font-light">
              Navegación
            </h4>
            <nav className="flex flex-col gap-3">
              {[
                { href: "/#inicio", label: "Inicio" },
                { href: "/#nosotros", label: "Nosotros" },
                { href: "/#servicios", label: "Tratamientos" },
                { href: "/#resultados", label: "Resultados" },
                { href: "/#testimonios", label: "Testimonios" },
                { href: "/blog", label: "Blog" },
                { href: "/#contacto", label: "Contacto" },
              ].map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-card/50 hover:text-primary transition-colors text-sm link-underline w-fit"
                >
                  {link.label}
                </Link>
              ))}
            </nav>
          </div>

          {/* Treatments */}
          <div>
            <h4 className="text-[10px] tracking-[0.3em] uppercase text-card/40 mb-5 font-light">
              Tratamientos
            </h4>
            <nav className="flex flex-col gap-3">
              {treatments.map((t) => (
                <Link
                  key={t}
                  href="#servicios"
                  className="text-card/50 hover:text-primary transition-colors text-sm link-underline w-fit"
                >
                  {t}
                </Link>
              ))}
            </nav>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-[10px] tracking-[0.3em] uppercase text-card/40 mb-5 font-light">
              Contacto
            </h4>
            <div className="space-y-3 text-sm text-card/50">
              <p>C. Turia, 30<br />41900 Camas, Sevilla</p>
              <p>Lun – Vie: 10:00 – 20:00<br />Sáb: 10:00 – 15:00</p>
              <a href="tel:+34622886878" className="hover:text-primary transition-colors block">
                622 886 878
              </a>
              <a href="mailto:hola@yulicolors.es" className="hover:text-primary transition-colors block">
                hola@yulicolors.es
              </a>
            </div>

            <Link
              href="https://wa.me/34622886878?text=Hola,%20me%20gustaría%20reservar%20una%20cita%20en%20YULI%20COLORS"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 mt-6 text-[11px] tracking-[0.15em] uppercase text-[#25D366]/70 hover:text-[#25D366] transition-colors duration-300"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
              </svg>
              WhatsApp
            </Link>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col md:flex-row items-center gap-3">
            <p className="text-card/30 text-xs tracking-wide">
              © {new Date().getFullYear()} YULI COLORS. Todos los derechos reservados.
            </p>
            <span className="text-card/20 text-xs hidden md:block">·</span>
            <p className="text-card/30 text-xs tracking-wide">
              Marketing por:{" "}
              <Link
                href="https://www.drastika.es/"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-primary/70 transition-colors"
              >
                DRSTK
              </Link>
            </p>
          </div>
          <div className="flex gap-6">
            <Link href="/privacidad" className="text-card/30 hover:text-primary/70 transition-colors text-xs tracking-wide">
              Política de Privacidad
            </Link>
            <Link href="/cookies" className="text-card/30 hover:text-primary/70 transition-colors text-xs tracking-wide">
              Política de Cookies
            </Link>
            <Link href="/aviso-legal" className="text-card/30 hover:text-primary/70 transition-colors text-xs tracking-wide">
              Aviso Legal
            </Link>
          </div>
        </div>
      </div>
    </footer>
  )
}
